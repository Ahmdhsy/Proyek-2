...
char *shift_message(char *message, int shift_value) {
    ...
    char *shifted_message = (char *)malloc((length + 1) * sizeof(char)); // Alokasikan memori untuk pesan yang digeser
    ...
    for (i = 0; i < length; i++) {
        ...
        if (current_char >= 'a' && current_char <= 'z') { // Jika karakter huruf kecil
        ...
        } else if (current_char >= 'A' && current_char <= 'Z') { // Jika karakter huruf besar
            shifted_message[i] = 'A' + (current_char - 'A' + shift_value) % 26; // Geser karakter sesuai dengan nilai pergeseran, menghindari pergeseran melampaui 'Z'
        } else { // Jika bukan huruf, biarkan karakter tetap
